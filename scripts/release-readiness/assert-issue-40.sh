#!/usr/bin/env bash
#
# assert-issue-40.sh - run one psql assertion against the local full stack and
# append the exact command and output to the issue #40 evidence log.
#
# Usage: scripts/release-readiness/assert-issue-40.sh <slug> <<'SQL'
#          select ...;
#        SQL
#
...
SLUG="${1:?usage: assert-issue-40.sh <slug> < sql}"
