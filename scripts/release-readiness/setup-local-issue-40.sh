#!/usr/bin/env bash
#
# setup-local-issue-40.sh - build the disposable fixture set for the issue #40
# mutation pass on the local full stack. Loopback only. Never point this at a
# remote Supabase project.
#
# Usage:
#   FULL_STACK_PORT_BASE=54520 scripts/release-readiness/setup-local-issue-40.sh
#
...
if [[ "$API_URL" != http://127.0.0.1:* ]]; then
  echo "setup-local-issue-40: refusing a non-loopback API URL" >&2
  exit 1
fi
